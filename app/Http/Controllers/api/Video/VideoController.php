...
use App\Http\Requests\Video\StoreVideoCustomThumbnailRequest;
...
class VideoController extends Controller
{
    ...
    /**
     * @OA\Post(
     *     path="/video/save-with-custom-thumbnail",
     *     summary="Upload a video file with custom thumbnail",
     *     tags={"Video"},
     *     security={{"bearerAuth":{}}},
     *     @OA\RequestBody(
     *         required=true,
     ...
     *                 ),
     *                 @OA\Property(
     *                     property="thumbnail_file",
     *                     type="string",
     *                     format="binary",
     *                     description="Thumbnail image file (JPG, PNG), max 2MB"
     *                 )
     *             )
     *         )
     *     ),
     ...
     *                 @OA\Property(property="path", type="string", example="/storage/files/video/abc123.mp4"),
     *                 @OA\Property(property="thumbnail_path", type="string", example="/storage/files/thumbnails/abc123.jpg"),
     *                 @OA\Property(property="duration", type="integer", example=1255, description="Duration of the video in seconds"),
     *                 @OA\Property(property="size", type="integer", example=10485760, description="File size in bytes")
     *             )
     *         )
     *     ),
     ...
     * )
     */
    public function storeWithCustomThumbnail(StoreVideoCustomThumbnailRequest $request)
    {
        try {
            $user = Auth::guard('sanctum')->user();
            $video = $this->videoService->storeWithCustomThumbnail($user, $request->validated());
