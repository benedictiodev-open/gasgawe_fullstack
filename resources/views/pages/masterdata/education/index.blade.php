...
@section('main')
  <div x-data="educationModalHandler()" x-init>
    ...
    {{-- MODAL --}}
    @include('pages.masterdata.education.form')
  </div>
@endsection
...
@push('script')
  <script>
    ...
    function educationModalHandler() {
      return {
        deleteEducationId: null,

        openCreateModal() {
          const dialog = document.getElementById('showCreateEducationModal');
          if (dialog) {
        ...
        openEditModal(education) {
          ...
          if (dialog) {
            ...
            form.action = `{{ route('masterdata.education.update', ':id') }}`.replace(':id', education.id);
            ...
            // Set values
            form.querySelector('[name="name"]').value = education.name;
            form.querySelector('[name="description"]').value = education.description;
            const isActiveRadios = form.querySelectorAll('input[name="is_active"]');
            isActiveRadios.forEach(radio => {
              radio.checked = radio.value === String(education.is_active);
            });
        ...
        openDeleteModal(id) {
          ...
          const dialog = document.getElementById('showDeleteEducationModal');
          dialog?.showModal();
        }
      }
    }
