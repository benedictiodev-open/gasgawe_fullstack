...
@section('main')
  <div x-data="employmentTypeModalHandler()" x-init>
    ...
    {{-- MODAL --}}
    @include('pages.masterdata.employment-type.form')
  </div>
@endsection
...
@push('script')
  <script>
    ...
    function employmentTypeModalHandler() {
      return {
        deleteEmploymentTypeId: null,

        openCreateModal() {
          const dialog = document.getElementById('showCreateEmploymentTypeModal');
          if (dialog) {
        ...
        openEditModal(employmentType) {
          const dialog = document.getElementById('showCreateEmploymentTypeModal');
          if (dialog) {
            // Update form action to PUT
            const form = dialog.querySelector('form');
            form.action = `{{ route('masterdata.employmentType.update', ':id') }}`.replace(':id', employmentType.id);
            ...
            // Set values
            form.querySelector('[name="name"]').value = employmentType.name;
            form.querySelector('[name="description"]').value = employmentType.description;
            const isActiveRadios = form.querySelectorAll('input[name="is_active"]');
            isActiveRadios.forEach(radio => {
              radio.checked = radio.value === String(employmentType.is_active);
            });
        ...
        openDeleteModal(id) {
          this.deleteEmploymentTypeId = id;
          const dialog = document.getElementById('showDeleteEmploymentTypeModal');
          dialog?.showModal();
        }
      }
    }
