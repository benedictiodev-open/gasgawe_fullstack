...
@section('main')
  <div x-data="expectedSalaryModalHandler()" x-init>
      ...
      <div class="col-span-12">
        <div class="overflow-x-auto">
          <table class="table border-separate border-spacing-y-2">
            ...
            <tbody>
              @foreach ($expectedSalaries as $item)
    ...
    {{-- MODAL --}}
    @include('pages.masterdata.expected-salary.form')
  </div>
@endsection
...
@push('script')
  <script>
    ...
    function expectedSalaryModalHandler() {
      return {
        deleteExpectedSalaryId: null,

        openCreateModal() {
          const dialog = document.getElementById('showCreateExpectedSalaryModal');
          if (dialog) {
        ...
        openEditModal(expectedSalary) {
          const dialog = document.getElementById('showCreateExpectedSalaryModal');
          if (dialog) {
            // Update form action to PUT
            const form = dialog.querySelector('form');
            form.action = `{{ route('masterdata.expectedSalary.update', ':id') }}`.replace(':id', expectedSalary.id);
            ...
            // Set values
            form.querySelector('[name="name"]').value = expectedSalary.name;
            form.querySelector('[name="description"]').value = expectedSalary.description;
            const isActiveRadios = form.querySelectorAll('input[name="is_active"]');
            isActiveRadios.forEach(radio => {
              radio.checked = radio.value === String(expectedSalary.is_active);
            });
        ...
        openDeleteModal(id) {
          this.deleteExpectedSalaryId = id;
          const dialog = document.getElementById('showDeleteExpectedSalaryModal');
          dialog?.showModal();
        }
      }
    }
