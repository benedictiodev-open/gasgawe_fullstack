...
@section('main')
  <div x-data="skillModalHandler()" x-init>
                    ...
                    <div class="flex flex-row items-center gap-4">
                      <button class="cursor-pointer"
                        @click="openEditModal({ 
                          id: {{ $item->id }}, 
                          name: '{{ $item->name }}', 
                          skill_group_id: {{ $item->skill_group_id }} 
                        })">
    ...
    {{-- MODAL --}}
    @include('pages.masterdata.skills.form')
  </div>
@endsection
...
@push('script')
  <script>
    ...
    function skillModalHandler() {
      return {
        deleteSkillId: null,

        openCreateModal() {
          const dialog = document.getElementById('showCreateSkillModal');
          if (dialog) {
        ...
        openEditModal(skill) {
          ...
          if (dialog) {
            ...
            form.action = `{{ route('masterdata.skills.update', ':id') }}`.replace(':id', skill.id);
            ...
            form.querySelector('[name="skill_group_id"]').value = skill.skill_group_id;
        ...
        openDeleteModal(id) {
          ...
          const dialog = document.getElementById('showDeleteSkillModal');
          dialog?.showModal();
        }
      }
    }
